...
##########################################################
	...
	def statement(self):
		...
		if res.error:
			return res.failure(InvalidSyntaxError(
				self.current_tok.pos_start, self.current_tok.pos_end,
				"Expected 'RETURN', 'CONTINUE', 'BREAK', 'VAR', 'IF', 'FOR', 'WHILE', 'FUN', int, float, identifier, '+', '-', '(' or '['"
			))
	...
	def atom(self):
		...
		if tok.type == TT_STRING:
			...
			func_def = res.register(self.while_expr())
			if res.error:
				return res
		...
		return res.failure(InvalidSyntaxError(
			...
			"Expected int, float, identifier, '+', '-' or '(', 'p' 'IF', 'FOR', 'WHILE', 'FUN'"
		))
	...
	def expr(self):
		...
		if res.error:
			return res.failure(InvalidSyntaxError(self.current_tok.pos_start, self.current_tok.pos_end,
				"Expected 'VAR',  int, float, identifier, '+', '-', or '(', '[' 'IF', 'FOR', 'WHILE', 'FUN'"
			))
...
class BuiltInFunction(BaseFunction):
	...
	def execute_is_function(self, exec_context):
		is_number = isinstance(
			exec_context.symbol_table.get('value'), BaseFunction)
		return RTResult().success(Number.true if is_number else Number.false)
...
class Interpreter:
	...
	def visit_ForNode(self, node, context):
		...
		while condition():
			...
			elemenmts.append(value)

		return res.success(
			Number.null if node.should_return_null else
			List(elemenmts).set_context(context).set_pos(
				node.pos_start, node.pos_end)
		)
	...
	def visit_CallNode(self, node, context):
		...
		if res.should_return():
			return res
		returb_value = return_value.copy().set_pos(
